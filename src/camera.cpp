...
Camera::Camera(int map_width, int map_height, int view_width, int view_height, sf::Vector2f view_location) {
    ...
    this->view = sf::View(sf::FloatRect(sf::Vector2f((view_location.x + 32) - view_width/2.0, (view_location.y + 32)- view_height/2.0), sf::Vector2f(view_width, view_height)));
}
...
void Camera::checkBoxesCollision(sf::RectangleShape BB) {
    ...
    else if(this->view.getCenter().x > (this->map.getGlobalBounds().size.x - this->view.getSize().x/2.0)) {
        this->view.move(sf::Vector2f((this->map.getGlobalBounds().size.x - this->view.getSize().x/2.0) - this->view.getCenter().x, 0.0));
    }
    else if(this->view.getCenter().y > (this->map.getGlobalBounds().size.y - this->view.getSize().y/2.0)) {
        this->view.move(sf::Vector2f(0.0, (this->map.getGlobalBounds().size.y - this->view.getSize().y/2.0) - this->view.getCenter().y));
    }
